...
-- Clue #2: Now that we're here, we havqe insight that Carmen was seen attending language classes in
...
WHERE region = 'South America' AND c.name LIKE 'Serra%';
...
-- Write SQL query here
 ...
 SELECT c.name
 FROM city c
 LEFT JOIN country ON c.id = country.capital 
